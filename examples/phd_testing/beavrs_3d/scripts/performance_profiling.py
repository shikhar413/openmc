...
def generate_input_files(cluster, prob_type, run_file):
    if cluster == "Green Cluster":
        param_dict = {
            ...
            '3d-beavrs': {
                'batch_file': "job.slurm",
                'params_file': '3db-params.cfg',
                'partition': 'sched_mit_nse',
                'run_command': "sbatch ",
                'base_files': ['3db-settings.xml', '3db-geometry.xml', '3db-materials.xml'],
                'tasks_per_node': 32,
                'procs_per_node': 2,
                'walltime': '12:00:00',
                'mult_factor': 4,
    ...
    for run_strat in run_strats:
        ...
        if test_num == 6 and prob_type in ['2d-beavrs', '3d-beavrs']:
            continue
        ...
        if ea_run:
            os.system('cp ./../../../base/run_openmc_ea_pp.py ./run_openmc.py')
        else:
            os.system('cp ./../../../base/run_openmc_noea_pp.py ./run_openmc.py')
...
def create_files(batch_template, params_template, cluster_params, prob_name, run_file, run_strat):
    ...
    os.system("sed -i 's~\/entropy_mesh~\/entropy_mesh--~g' settings.xml".format(n_inactive))
    os.system("sed -i 's~mesh id~!--mesh id~g' settings.xml".format(n_inactive))
...
if __name__ == "__main__":
    if len(sys.argv) not in [2, 3]:
        print('Usage: performance_profiling.py [prob_type] [-r]')
        sys.exit()

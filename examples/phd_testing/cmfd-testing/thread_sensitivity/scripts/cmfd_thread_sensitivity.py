...
def generate_input_files(cluster, run_file):
    ...
    os.system('mkdir -p ./../cmfd-thread-sensitivity')
    os.chdir('./../cmfd-thread-sensitivity')
    ...
    for mesh in cluster_params['mesh_types']:
        ...
        # TODO add 11 nodes explicitly for pincell CMFD mesh
        for node in nodes:
...
def create_files(param_template, batch_template, cluster_params, node, thread, run_file, mesh):
    ...
    # TODO set batches and inactive differently for pincell mesh
    param_template = param_template.replace('{n_batches}', str(cluster_params['n_batches']))
    param_template = param_template.replace('{n_inactive}', str(cluster_params['n_inactive']))

    with open('params.cfg', 'w') as f:
        f.write(param_template)

    print_str = os.getcwd().split('cmfd-thread-sensitivity/')[-1]
    if run_file:
